...
class Interface
  ...
  def wagons_manage
    ...
    if wagons[wagon_index].class == PassengerWagon
      wagons[wagon_index].take_seat
      puts "Место успешно занято!"
    elsif wagons[wagon_index].class == CargoWagon
  ...
  def create_train
    ...
    raise TrainArgumentError, "Введён некорректный тип поезда!" unless [PassengerTrain::TYPE, CargoTrain::TYPE].include? train_type
  ...
  def show_wagons_info
    block1 = Proc.new do |wagon, index|
  ...
  def add_wagon_to_train(train_index)
    ...
    elsif trains[train_index].type == :passenger
